...
export class SignalRService {
  ...
  start(hubUrl: string){
    ...
    this._connection.onreconnecting(error => console.log("Reconnected"));
    this._connection.onclose(error => console.log("Close reconnection"));
  }

  invoke(procedureName: string, message: any, succesCallBack?: (value: any) => void, errorCallBack?: (error: any) => void){
